...
from algorithms.svea_cmid import SVEA
...
class SVEA_CMID(SVEA):
    def __init__(self, obs_shape, action_shape, cfg):
        ...
        self.cmid_discriminator = models_cmid.CMIDDiscriminator(action_shape, cfg).cuda()
    ...
    def cmid_update(self, obs, obs_aug, prev_obs, prev_action, prev_masks, logger, step):
        ...
        true_labels = torch.ones(z_repeated.shape[0], 1).cuda()
        shuffled_labels = torch.zeros(closest_z_shuffled.shape[0], 1).cuda()
        ...
        shuffled_labels = torch.zeros(z_repeated.shape[0], 1).cuda()
