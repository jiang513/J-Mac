...
class SAC(object):
	def __init__(self, obs_shape, action_shape, args):
		...
		self.encoder_optimizer = torch.optim.Adam(
			self.critic.encoder.parameters(), lr=args.encoder_lr, betas=(args.critic_beta, 0.999)
		)
	...
	def update_actor_and_alpha(self, obs, L=None, step=None, update_alpha=True):
		...
		if update_alpha:
			...
			wandb.log({"alpha_loss":alpha_loss},step=step)
	...
	def load(self, model_dir, step):
		self.load_state_dict(
			torch.load('%s/%s.pt' % (model_dir, step))
		)
