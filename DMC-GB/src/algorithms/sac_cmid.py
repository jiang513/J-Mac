...
import utils
from algorithms import models_cmid
import wandb

class SAC(object):
    def __init__(self, obs_shape, action_shape, cfg):
        self.cfg = cfg
        self.action_range = [cfg.action_range_low, cfg.action_range_high]
        ...
        self.actor = models_cmid.Actor(obs_shape, action_shape, cfg).cuda()

        self.critic = models_cmid.Critic(obs_shape, action_shape, cfg).cuda()
        self.critic_target = models_cmid.Critic(obs_shape, action_shape, cfg).cuda()
        self.critic_target.load_state_dict(self.critic.state_dict())

        # tie conv layers between actor and critic
        self.actor.encoder.copy_conv_weights_from(self.critic.encoder)

        self.log_alpha = torch.tensor(np.log(cfg.init_temperature)).cuda()
